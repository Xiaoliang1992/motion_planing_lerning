...
% START ALGORITHM
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
while(HavePath == 1 && (xNode ~= xTarget || yNode ~= yTarget))
    ...
    for i = 1:size(exp_n, 1)
        flag = 0;
        for j = 1:OPEN_COUNT
            if (exp_n(i,1) == OPEN(j,2) && exp_n(i,2) == OPEN(j,3)) % expanded node already in openlist
                if (exp_n(i,4) < OPEN(j,7))
                    % update parents
                    OPEN(j,4) = xNode;
                    OPEN(j,5) = yNode;
                    OPEN(j,7) = exp_n(i,4);
                    OPEN(j,8) = OPEN(j,7) + OPEN(j,6); % cause g(n), i.e. OPEN(j,7) updated by exp_n
                end
    ...
    % Remove the node n with lowest f(n)
